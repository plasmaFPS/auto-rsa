...
def get_history_data():
    ...
    for key, history in accounts.items():
        ...
        for entry in reversed(history):
            ...
            if entry["timestamp"] <= seven_days_ago and prev_7d_entry is None:
                prev_7d_entry = entry
            if entry["timestamp"] <= fourteen_days_ago and prev_14d_entry is None:
                prev_14d_entry = entry
        ...
        if prev_14d_entry:
            diff_14d = current_val - prev_14d_entry["value"]
            total_two_week_diff += diff_14d
        ...
        account_details.append({
            ...
            "has_14d": prev_14d_entry is not None,
    ...
    return {
        ...
        "total_two_week_diff": total_two_week_diff,
...
def get_history_report():
    ...
    for acc in data["accounts"]:
        ...
        diff_14d_str = "N/A"
        if acc["has_14d"]:
            sign = "+" if acc["diff_14d"] >= 0 else "-"
            diff_14d_str = f"{sign}${abs(acc['diff_14d']):.2f}"
        ...
        report_lines.append(f"  14 Days: {diff_14d_str}")
    ...
    report_lines.append(f"Total 14 Day Change: {sign_two_week}${abs(data['total_two_week_diff']):.2f}")
